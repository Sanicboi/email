...
class AuthManager {
    ...
    public async init() {
        this._pass = await fs.readFile(path.join(process.cwd(), process.env.PASSWD_PATH!), 'utf-8');
    }
    ...
    public async authorize(req: Request, res: Response, next: NextFunction): Promise<void> {
        try {
            ...
            if (!header.startsWith("Bearer ")) throw new Error("Authorization is not bearer");
    ...
    public async getToken(req: Request<any, any, {
        password: unknown
