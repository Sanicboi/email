...
export const poll = async (userCallback: (user: User, data: IIncomingMail) => Promise<void>): Promise<void> => {
    ...
    try {
        for await (const message of imap.fetch({
            ...
            if (!message.envelope || !message.envelope.sender || !message.envelope.sender[0].address || !message.source) continue;
